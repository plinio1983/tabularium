...
type Step = '' | 'salesChannelId' | 'schedule' | 'credit';
const groups = [
  {name: 'salesChannelId', label: 'Canale di vendita', hint: 'Assegna un canale di vendita comune', icon: '🏷'},
...
export default function RecurringIncomeBulkEditModal({formId, action, channels, methods, banks}: {
  formId: string; action: string; channels: Option[]; methods: Option[]; banks: Option[];
}) {
    ...
    <div className="bulk-category-modal bulk-edit-fields-modal" role="dialog" aria-modal="true" aria-labelledby={`${formId}-bulk-edit-title`}>
      <div className="modal-toolbar-card toolbar-card">
        <div className="bulk-edit-toolbar-copy"><span className="bulk-edit-toolbar-kicker">Modifica multipla</span>
          <h2 id={`${formId}-bulk-edit-title`}>{step ? groups.find(group => group.name === step)?.label : 'Modifica entrate ricorrenti'}</h2>
      ...
      <div key={step} className={`bulk-edit-step bulk-edit-step-${direction}`}>
        {!step ? <div className="bulk-edit-form bulk-edit-choice-form">
        ...
        </div> : <form action={action} method="post" className={`bulk-edit-form ${step === 'salesChannelId' ? 'bulk-edit-category-form' : 'bulk-edit-accounting-form'}`}>
          ...
          <div className={step === 'salesChannelId' ? 'bulk-edit-category-content' : 'bulk-edit-accounting-content'}>
            {step === 'salesChannelId' ? optionSelect('salesChannelId', 'Canale di vendita', channels) : null}
            {step === 'schedule' ? <>
            ...
            {step === 'credit' ? <>
              <EditableField title="Modalità accredito" hint="Scegli accredito manuale o automatico" name="updateCredit" active={!!active.updateCredit} onChange={next => toggle('updateCredit', next)}>
                <label><span>Nuova modalità accredito</span><select name="isAutomaticCredit" required value={value('isAutomaticCredit')} onChange={event => change('isAutomaticCredit', event.target.value)}>
            ...
            {step !== 'salesChannelId' && !enabledFields.some(name => active[name]) ? <p className="muted bulk-edit-dates-hint">Attiva almeno un campo da modificare.</p> : null}
          </div>
