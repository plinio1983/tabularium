...
export default function NewClientPanel({ initialOpen = false }: { initialOpen?: boolean }) {
  ...
  return <>{open ? <div className="modal-backdrop app-form-modal" role="dialog" aria-modal="true" aria-label="Aggiungi nuovo cliente" onMouseDown={() => setOpen(false)}><div className="modal-card modal-card-wide entity-form-modal-card" onMouseDown={event => event.stopPropagation()}>
    ...
    <form className="card form app-record-form entity-form entity-styled-form inline-create-form" action={action} method="post"><ClientFormFields /><div className="full actions-row form-actions-row form-sticky-actions entity-form-actions"><button className="btn btn-md btn-default" type="button" onClick={() => setOpen(false)}><span className="btn-icon">✕</span> Annulla</button><button className="btn btn-md btn-primary" type="submit"><span className="btn-icon">✓</span> Salva cliente</button></div></form>
  </div></div> : null}</>;
}
