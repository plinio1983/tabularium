...
export default function RecurringExpensesList({
    ...
    const mobileSortedItems = [...items].sort((a, b) => {
        ...
        switch (mobileSort) {
            ...
            case 'supplier_asc':
                return compareText(supplierA, supplierB, 'asc');
            case 'supplier_desc':
                return compareText(supplierA, supplierB, 'desc');
            ...
            case 'dueDay_asc':
                return compareNumber(a.dueDay, b.dueDay, 'asc');
            case 'dueMonth_asc':
                return compareNumber(a.dueMonth, b.dueMonth, 'asc');
            case 'billingPeriodMode_asc':
                return compareText(a.billingPeriodMode, b.billingPeriodMode, 'asc');
            case 'billingMonth_asc':
                return compareNumber(a.billingMonth, b.billingMonth, 'asc');
            ...
            case 'bank_asc':
                return compareText(a.bank?.name, b.bank?.name, 'asc');
            case 'declared_desc':
                ...
                <table className="expenses-table compact-recurring-expenses-table">
                    ...
                    <tr>
                        <th className="cell-center">
                            <input type="checkbox" className="bulk-select-all" data-bulk-target="recurringExpenseBulkForm" aria-label="Seleziona tutte le uscite ricorrenti"/>
                        </th>
                    ...
                    {items.map(item => {
                        ...
                        return <tr className="clickable-desktop-row" data-row-href={`/recurring-expenses/${item.id}?returnTo=${returnTo}`} tabIndex={0} key={item.id}>
                            ...
                            <td className="cell-left">
                                <RecurringStateToggle variant="button" kind="expense" id={item.id} active={item.isActive} archived={Boolean(item.archivedAt)} returnTo={returnTo}/>
                            </td>
                            ...
                            <td className="cell-left">{item.category ?
                                <span title={item.category.name} className={badgeClass(categoryClassName)}>{categoryLabel(item.category, item.category.code)}</span> :
                                <span className={badgeClass('tone-neutral')}>  •   ND</span>}</td>
                            <td className="cell-right nowrap-cell">
                                <strong className="recurring-table-amount">€ {euro(item.amount.toString()).replace('€', '').trim()}</strong>
                            </td>
                            <td className="cell-left">
                                <span className={badgeClass(cadenceStyle.className)}>{cadenceStyle.icon} {cadenceLabels[item.cadence] ?? item.cadence}</span>
                            </td>
                            <td className="cell-left nowrap-cell">
                                <span className={badgeClass('tone-waiting')}>📅 {dueLabel(item)}</span></td>
                            <td className="cell-left nowrap-cell">
