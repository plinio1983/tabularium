...
export default function RecurringDetailState({id, active, archived, variant}: {
  id: number; active: boolean; archived: boolean; variant: 'badge' | 'summary' | 'text' | 'progress';
}) {
  const {records} = useRecurringStates();
  const state = records[`expense:${id}`] ?? {active, archived};
