...
export default function ExpenseCreationSwitcher(props: Props) {
  ...
  function recurrenceControl(location: "external" | "internal") {
    return <div className={`app-form-field full app-form-wizard-step app-form-wizard-step-1 switch-toggle-field switch-inline wide push-down expense-recurring-definition-toggle expense-recurring-definition-toggle-${location}`}>
      <div className="switch-toggle-field-label app-form-field-label"><span className="app-form-field-icon" aria-hidden="true">↻</span><span className="app-form-label">Definizione ricorrente</span></div>
  ...
  const typeStep = <section className={`form app-record-form single-expense-form expense-creation-type-step app-form-wizard app-form-wizard-current-1 ${typeConfirmed ? "is-complete" : ""}`}>
    <div className="app-form-wizard-header full">
      <div className="app-form-wizard-heading"><span>Passaggio 1 di {totalSteps}</span><strong><span>Tipo di spesa</span></strong></div>
      <div className="app-form-wizard-progress" aria-label={`Passaggio 1 di ${totalSteps}`}><span style={{width: `${100 / totalSteps}%`}}/></div>
    </div>
