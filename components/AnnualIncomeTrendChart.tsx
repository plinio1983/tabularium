...
export default function AnnualIncomeTrendChart({initialData}: {initialData: IncomeTrendData}) {
  ...
  return <section className="card annual-income-trend-card">
    <div className="card-heading-row annual-income-trend-heading">
      ...
      <div className="annual-income-trend-controls">
        ...
        <div className="expense-trend-mode-toggle" role="group" aria-label="Modalità grafico">
          <button type="button" className={`expense-trend-mode-button ${mode === 'periodic' ? 'is-active' : ''}`} onClick={() => {setMode('periodic'); setSelectedIndex(null);}}>Periodico</button>
          <button type="button" className={`expense-trend-mode-button ${mode === 'cumulative' ? 'is-active' : ''}`} onClick={() => {setMode('cumulative'); setSelectedIndex(null);}}>Cumulativo</button>
        </div>
      </div>
    </div>
    ...
    <div className="annual-income-trend-kpis">
      ...
      <div><span>Media per intervallo</span><strong>{euro.format(data.points.length ? data.total / data.points.length : 0)}</strong></div>
      <div><span>Intervallo migliore</span><strong>{peak ? `${peak.label} · ${euro.format(peak.value)}` : '—'}</strong></div>
    </div>
    ...
    <div className={`annual-income-trend-scroll ${loading ? 'is-loading' : ''}`}>
      <svg className="annual-income-trend-chart" viewBox={`0 0 ${width} ${height}`} style={{width}} role="img" aria-label={`Andamento incassi ${data.year}, intervallo ${interval}`}>
        ...
        {data.points.map((point, index) => <g key={point.from}>
          {(index % labelEvery === 0 || index === data.points.length - 1) ? <text x={x(index)} y={height - 14} textAnchor="middle" className="annual-income-trend-date-label">{point.label.split(' – ')[0]}</text> : null}
    ...
    {!data.points.length ? <p className="muted annual-income-trend-empty">Nessun mese concluso disponibile per l’anno selezionato.</p> : null}

    <div className={`annual-income-trend-selection ${selected ? '' : 'is-empty'}`}>
      {selected ? <>
        <div><span>Periodo</span><strong>{selected.label}</strong></div>
        <div><span>{mode === 'cumulative' ? 'Cumulato' : 'Incassato'}</span><strong>{euro.format(mode === 'cumulative' ? selected.cumulative : selected.value)}</strong></div>
