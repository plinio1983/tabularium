...
export default function BulkExpenseAttachmentsModal({
    ...
    useEffect(() => {
        ...
        return () => {
            document.body.classList.remove('bulk-attachments-modal-open');
    ...
    async function archive(action: 'DOWNLOAD' | 'SHARE') {
        ...
        try {
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({ids, filter, action})
            });
            if (!response.ok) {
                const body = await response.json().catch(() => null);
            ...
            if (action === 'SHARE' && navigator.share && (!navigator.canShare || navigator.canShare({files: [file]}))) {
                await navigator.share({
                    title: `Allegati ${subject}`,
                    text: `Allegati degli elementi selezionati`,
                    files: [file]
                });
    ...
    async function sendEmail() {
        if (!email.trim()) {
            setError('Inserisci l’indirizzo email del destinatario.');
            return;
        }
        setBusy('EMAIL');
        setError('');
        ...
        try {
            ...
            setError(cause instanceof Error ? cause.message : 'Invio email non riuscito');
        } finally {
            setBusy(null);
        }
    }
    ...
    const modal = open ? createPortal(
        <div className="app-form-modal-backdrop bulk-attachments-modal-backdrop" role="presentation"
             onMouseDown={event => event.target === event.currentTarget && setOpen(false)}>
            <section className="app-form-modal bulk-attachments-modal" role="dialog" aria-modal="true"
                     aria-labelledby={`${formId}-attachments-title`}>
                ...
                <div className="btn-group bulk-attachment-filter-group" role="group" aria-label="Tipo allegati">
                    {filters.map(option => <button type="button"
                                                   className={filter === option.value ? 'is-selected' : ''}
                                                   aria-pressed={filter === option.value} key={option.value}
                                                   onClick={() => {
                ...
                <div className="bulk-attachments-summary" aria-live="polite">
                    {busy === 'SUMMARY' ? <p>Verifica degli allegati…</p> : summary ? <>
                        ...
                        <div>
                            <span>Dimensione</span><strong>{summary.totalBytes ? formatBytes(summary.totalBytes) : '—'}</strong>
                        </div>
                    </> : null}
                </div>

                <label className="app-form-field bulk-attachments-email-field">
                    <span className="app-form-field-label"><span className="app-form-field-icon"
                                                                 aria-hidden="true">@</span>Destinatario email</span>
                ...
                {!busy && summary && !summary.attachmentCount ?
                    <p className="inline-warning bulk-attachments-feedback">Nessun allegato del tipo
                        selezionato.</p> : null}
                ...
                <div className="actions-row form-actions-row bulk-attachments-actions">
                    ...
                    <button className="btn btn-md btn-default" type="button" disabled={!available || !email.trim()}
                        ...
                        className="btn-icon">✉</span>{busy === 'EMAIL' ? 'Invio…' : 'Invia mail'}</button>
                    <button className="btn btn-md btn-default" type="button" disabled={!available}
                            onClick={() => archive('SHARE')}><span
                        className="btn-icon">↗</span>{busy === 'SHARE' ? 'Preparazione…' : 'Condividi'}</button>
                    <button className="btn btn-md btn-primary" type="button" disabled={!available}
                            onClick={() => archive('DOWNLOAD')}><span
                        className="btn-icon">⇩</span>{busy === 'DOWNLOAD' ? 'Preparazione…' : 'Scarica'}</button>
                </div>
            </section>
        </div>, document.body
    ) : null;
