...
export default function IncomeSalesChannelTrendChart({initialData, availableYears}: {
    ...
    const periodChannels = data.channels.map(channel => ({
        ...channel,
        total: buckets.reduce((sum, bucket) => sum + (bucket.channels.find(item => item.id === channel.id)?.total ?? 0), 0)
    })).filter(channel => channel.total > 0).sort((a, b) => b.total - a.total);
    ...
    return <section id="incassi" className="income-channel-trend-card dashboard-anchor-section" aria-labelledby="income-channel-trend-title">
            ...
            <div className="income-channel-trend-controls">
                ...
                <div className="trend-mode-toggle income-channel-period-toggle" role="group" aria-label="Periodo grafico">
                    {([['year', 'Anno'], ['q1', 'Tri 1'], ['q2', 'Tri 2'], ['q3', 'Tri 3'], ['q4', 'Tri 4']] as const).map(([value, label]) =>
                        <button type="button" key={value} className={`trend-mode-button ${period === value ? 'is-active' : ''}`} onClick={() => setPeriod(value)}>{label}</button>)}
                </div>
                {availableYears.length > 1 ?
                    <select aria-label="Anno del grafico" value={data.year} disabled={loading} onChange={event => void changeYear(Number(event.currentTarget.value))}>
        ...
        <div className="income-channel-trend-kpis">
            ...
            <div>
                <span>Canale principale</span><strong>{leading ? `${leading.icon ?? '•'} ${leading.name}` : '—'}</strong>
            </div>
            <div>
                <span>{period === 'year' ? 'Mese migliore' : 'Settimana migliore'}</span><strong>{peak && peak.total > 0 ? `${peak.label} · ${money.format(peak.total)}` : '—'}</strong>
            </div>
        </div>
            ...
            <div className={`income-channel-trend-plot ${period === 'year' ? '' : 'is-quarter'}`} role="img" aria-label={`Andamento incassi ${data.year}, ${period === 'year' ? 'anno' : period}`}>
                {buckets.map(bucket => <div className="income-channel-month" key={bucket.from}>
                    ...
                    <div className="income-channel-bar-stage">
                        {bucket.total > 0 ? view === 'total' ?
                            <Link href={rangeHref(bucket.from, bucket.to)} className="income-channel-total-bar" style={{height: `${bucket.total / max * 100}%`}} title={`${bucket.label}: ${money.format(bucket.total)}`} aria-label={`Vedi incassi di ${bucket.label}: ${money.format(bucket.total)}`}/> :
                            <div className="income-channel-stacked-bar" style={{height: `${bucket.total / max * 100}%`}}>
                                {bucket.channels.map(channel =>
                                    <Link key={channel.id} href={rangeHref(bucket.from, bucket.to, channel.name)} style={{
                                        height: `${channel.total / bucket.total * 100}%`,
                                        background: channelColors.get(channel.id)
                                    }} title={`${channel.name}, ${bucket.label}: ${money.format(channel.total)}`} aria-label={`Vedi ${channel.name} di ${bucket.label}: ${money.format(channel.total)}`}/>)}
                            </div> : null}
                    </div>
        ...
        {view === 'channels' ? <div className="income-channel-trend-legend" aria-label="Legenda canali">
            {periodChannels.map(channel => <div key={channel.id}>
                <span style={{background: channelColors.get(channel.id)}}/>
                <strong>{channel.icon ?? '•'} {channel.name}</strong>
                <div className="income-channel-item-values">
                    <span className="income-trend-money">{money.format(channel.total)}</span> ·
                    <strong className="income-trend-percent">{periodTotal ? (channel.total / periodTotal * 100).toFixed(1) : '0.0'}%</strong>
                </div>
            </div>)}
        </div> : null}
    </section>;
