...
export async function GET(_: Request, {params}: {params: Promise<{id: string}>}) {
  ...
  if (!Number.isInteger(id) || id <= 0) return NextResponse.json({error: 'Allegato non valido'}, {status: 400});
  const attachment = await prisma.incomeAttachment.findFirst({where: {id, income: {workspaceId: current.workspace.id, companyId: current.company.id}}});
