...
export default async function RecurringIncomesPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="grid"><NewIncomePanel showToolbar={false} banks={banks.map(bank=>({...bank,isPrimary:bank.id===current.company.primaryBankId}))} paymentMethods={methods} salesChannels={channels} customers={customers} /><div className="toolbar-card expense-toolbar-card"><div><h2>Entrate ricorrenti</h2><p className="muted">Gestisci le regole che generano periodicamente gli incassi.</p></div><button className="btn btn-sm btn-primary income-add-btn" type="button" data-income-new data-income-new-type="recurring"><span className="btn-icon">＋</span>Nuova entrata ricorrente</button></div>
