...
export default async function EditRecurringIncomePage({ params, searchParams }: { params: Promise<{id:string}>; searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [item,channels,customers,methods,banks]=await Promise.all([
    prisma.recurringIncome.findFirst({where:{id,workspaceId,companyId:current.company.id}}), prisma.incomeSalesChannel.findMany({where:{workspaceId},orderBy:{sortOrder:'asc'}}), prisma.customer.findMany({where:{workspaceId},orderBy:{businessName:'asc'}}), prisma.paymentMethod.findMany({where:{workspaceId,kind:{in:['INCOME','BOTH']}},orderBy:{name:'asc'}}), prisma.bank.findMany({where:{workspaceId},orderBy:{name:'asc'}})
  ]); if(!item) notFound();
