...
function CopyableField({label, value, className = ''}: { label: string; value?: string | null; className?: string }) {
    ...
    return <div className={`${className} copyable-detail-field`}>
        <span>{label}</span><strong className={label === 'Note interne' ? 'displayed-notes' : undefined}>{display}</strong><CopyValueButton value={display === '-' ? '' : display}/>
    </div>;
}
                ...
                <section className="record-detail-hero">
                    <div>
                        <div className="record-detail-title-block">
                            <p className="record-detail-kicker">Cliente #{customer.id}</p>
                            <h1>{customer.businessName}</h1>
                            <div className="record-detail-meta-line">
                                <span>{valueOrDash(customer.alias)}</span><span className="badge">{customer.incomes.length} incassi collegati</span>
                            </div>
                        </div>
                    </div>
                    <aside className="record-detail-amount-panel">
                        <div className="record-detail-amount-panel-header-row">
                            <span className="record-detail-amount-panel-header">Da accreditare</span></div>
                        <strong className={uncreditedTotal > 0 ? 'text-warning' : 'text-ok'}>{euro(uncreditedTotal)}</strong>
                        <div className="record-detail-badge-row">
                            <span className={badgeClass(uncreditedTotal > 0 ? incomeCreditStatusStyles.DA_ACCREDITARE.className : incomeCreditStatusStyles.ACCREDITATO.className)}>{uncredited.length} incassi aperti</span>
                        </div>
                    </aside>
                </section>
                <section className="record-detail-status-strip">
                    <div><span>Incassi collegati</span><strong>{customer.incomes.length}</strong></div>
                    <div><span>Da accreditare</span><strong>{uncredited.length}</strong></div>
                    <div>
                        <span>Importo da accreditare</span><strong className={uncreditedTotal > 0 ? 'text-warning' : 'text-ok'}>{euro(uncreditedTotal)}</strong>
                    </div>
                    <div><span>Incassati {currentYear}</span><strong>{euro(annualTotal)}</strong></div>
                </section>
